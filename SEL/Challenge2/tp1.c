...
void modifMemAppel(int num_pid,long unsigned adrMem){
	char appel[3] = {0xFF,0xD0,0xCC};
	FILE *chemin;
	char cmd[MAX_LENGTH];

	// cmd= est la commande qui permet d'avoir acces a la memoire du programme dans le but de la modifier par la suite
	snprintf(cmd,sizeof("/proc//mem")+sizeof(num_pid)+1,"/proc/%d/mem",num_pid);


	chemin = fopen(cmd,"w");// ouvre ce fichier system
	if (chemin == NULL){
		perror("fopen erreur pour ouverture du chemin /proc/pid/mem \n");
		exit(0);
	}
	...
	if(fwrite(appel,1,1,chemin)<0){ // on ecrit le code assemble qui veut interruption dans cette memoire
		perror("fwrite erreur\n");
		exit(0);
	}
...
//******************************** recupRegistre *************************//
struct user_regs_struct recupRegistre (int num_pid,struct user_regs_struct registre){
	long regs;
	regs = ptrace( PTRACE_GETREGS, num_pid, NULL, &registre);

	if(regs<0){
		perror("erreur sur la recuperation de registre");
		exit(0);
	}
	return registre;
}
...
struct user_regs_struct modifRegistreTmp (int num_pid, struct user_regs_struct registre ){
	long regs;
	regs = ptrace( PTRACE_SETREGS, num_pid, NULL, &registre);
	
	registre.rax = @addr de la fonction que l'on veut appelé (qui se trouve en mémoire);
	registre.rip = @(0xFFD0CC) addr ou se trouve la ligne de code 0xFFD0CC;
...
////////////////////////////////////////////////////////////Challenge 1/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//********************************Parti recuperation du pid*************************//
	num_pid = trouverPid(progr);

	attacherProc(num_pid)


	//********************************PARTIE DES ADRESSES MEMOIRE*************************//
	adrMem = trouverAdrMemoire(progr,fct);

	//********************************Suite*************************//

	modifMemStopper(num_pid,adrMem);
 ...
 //******************************** REGISTRE *************************//
	...
	registre = recupRegistre(num_pid,registre);

	registre = modifRegistreTmp(num_pid,registre);
	long modifRegistre = ptrace ( PTRACE_CONT, num_pid, NULL, &registre);
	...
	//TODO faire la suite
	//registre = posix_memalign();


	/*if ( posix_memalign (&memptr, getpagesize(), tailleDuProgramme) != 0){
		perror("posix_memalign erreur \n");
		return -1;
	}*/

	//car peux pas modifier toutes les plages d'adresse donc rend la zone autorisable
	if(mprotect(memptr, tailleDuProgramme, PROT_WRITE) !=0 ) {
		perror("mprotect erreur \n");
		return -1;
	}
